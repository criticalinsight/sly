...
* **Recursive Reflexion**: Autonomous self-healing OODA loops. If a command fails, Sly automatically spawns a sub-agent (`sly-run-task`) to analyze and fix the error. The agent can even **rewrite its own source code** and evolve via `sly_evolve`.
* **CLI-First Architecture**: Unified I/O via standard pipes allows seamless composition. Run `sly session "Do X"` and pipe the output to another tool.
