...
use crate::utils::is_gleam_project;
...
async fn handle_client_request(
    ...
    match request.method.as_str() {
        ...
        // Gleam inspection
        "_tidewave.ai/gleam/inspect" => handle_gleam_inspect(state, websocket_id, request).await,
        // Gleam fact sync
        "_tidewave.ai/gleam/fact/sync" => handle_gleam_fact_sync(state, websocket_id, request).await,
        // BEAM Observability
        "_tidewave.ai/beam/nodes" => handle_beam_nodes(state, websocket_id, request).await,
        "_tidewave.ai/beam/ets/list" => handle_beam_ets_list(state, websocket_id, request).await,
        "_tidewave.ai/beam/ets/view" => handle_beam_ets_view(state, websocket_id, request).await,
        // Exit request
...
async fn handle_initialize_request(
    ...
    let mut spawn_opts: TidewaveSpawnOptions = serde_json::from_value(spawn_opts.clone())
        .map_err(|e| anyhow::anyhow!("Invalid spawn options format: {}", e))?;

    // Gleam-specific optimizations
    let cwd_path = std::path::Path::new(&spawn_opts.cwd);
    if is_gleam_project(cwd_path) {
        debug!("Gleam project detected in {}", spawn_opts.cwd);

        // Smart Default: If command is just shell or empty, use 'gleam run'
        if spawn_opts.command.is_empty()
            || spawn_opts.command == "sh"
            || spawn_opts.command == "bash"
            || spawn_opts.command == "zsh"
        {
            spawn_opts.command = "gleam run".to_string();
        }

        // Environment injection
        spawn_opts
            .env
            .entry("ERL_AFLAGS".to_string())
            .or_insert("-kernel shell_history enabled".to_string());
    }
...
async fn handle_gleam_inspect(
    state: &AcpProxyState,
    websocket_id: WebSocketId,
    request: &JsonRpcRequest,
) -> Result<()> {
    // Get command and CWD from the process state associated with this websocket
    ...
    let process_state = state
        .processes
        .get(process_key.value())
        .ok_or_else(|| anyhow!("Process not found"))?;

    let cwd = &process_state.spawn_opts.cwd;

    // Command to gather BEAM info
    let erl_cmd = "erl -noshell -eval 'io:format(\"~p\", [{node(), length(processes()), erlang:memory(), length(ets:all())}]), init:stop().'";

    let mut command = create_shell_command(erl_cmd, HashMap::new(), cwd, false);
    let output = command
        .output()
        .await
        .map_err(|e| anyhow!("Failed to run inspection: {}", e))?;
    ...
    let response = JsonRpcResponse {
        jsonrpc: "2.0".to_string(),
        id: request.id.clone(),
        result: Some(serde_json::json!({ "inspection": result_str })),
        error: None,
    };
...
async fn handle_gleam_fact_sync(
    state: &AcpProxyState,
    websocket_id: WebSocketId,
    request: &JsonRpcRequest,
) -> Result<()> {
    let params = request.params.as_ref().ok_or_else(|| anyhow!("Missing params"))?;
    let facts = params.get("facts").ok_or_else(|| anyhow!("Missing facts field"))?;
    ...
    // We'll write the facts to a temporary Erlang term file that the app can load
    // This is a robust way to bridge without needing a live connection immediately
    let facts_json = serde_json::to_string(facts)?;
    let facts_path = std::path::Path::new(cwd).join(".tidewave_facts.json");
    tokio::fs::write(&facts_path, facts_json).await?;

    debug!("Synced facts to {:?}", facts_path);

    let response = JsonRpcResponse {
        jsonrpc: "2.0".to_string(),
        id: request.id.clone(),
        result: Some(serde_json::json!({ "status": "synced", "path": ".tidewave_facts.json" })),
        error: None,
    };
...
async fn handle_beam_nodes(
    state: &AcpProxyState,
    websocket_id: WebSocketId,
    request: &JsonRpcRequest,
) -> Result<()> {
    // 1. Execute discovery
    let nodes = beam::list_nodes().await?;

    // 2. Respond
    let response = JsonRpcResponse {
        jsonrpc: "2.0".to_string(),
        id: request.id.clone(),
        result: Some(serde_json::to_value(nodes)?),
        error: None,
    };
...
async fn handle_beam_ets_list(
    state: &AcpProxyState,
    websocket_id: WebSocketId,
    request: &JsonRpcRequest,
) -> Result<()> {
    let params = request.params.as_ref().ok_or_else(|| anyhow!("Missing params"))?;
    let node = params.get("node").and_then(|v| v.as_str()).ok_or_else(|| anyhow!("Missing node param"))?;

    // 1. Execute inspection
    let result_str = beam::inspect_ets_list(node).await?;

    // 2. Respond (we send the raw string representation for now, or maybe wrap it)
    // The client (agent) can parse the Erlang term string.
    let response = JsonRpcResponse {
        jsonrpc: "2.0".to_string(),
        id: request.id.clone(),
        result: Some(serde_json::json!({ "output": result_str })),
        error: None,
    };
...
async fn handle_beam_ets_view(
    ...
    let params = request.params.as_ref().ok_or_else(|| anyhow!("Missing params"))?;
    let node = params.get("node").and_then(|v| v.as_str()).ok_or_else(|| anyhow!("Missing node param"))?;
    let table = params.get("table").and_then(|v| v.as_str()).ok_or_else(|| anyhow!("Missing table param"))?;

    // 1. Execute inspection
    let result_str = beam::inspect_ets_view(node, table).await?;
